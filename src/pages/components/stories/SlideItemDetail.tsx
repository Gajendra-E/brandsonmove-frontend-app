...
const SlideItemDetail: React.FC<SlideItemDetailProps> = ({
  ...
  const showContent = (story: any) => {
    return (
      <div
        className="info-animation-container"
        style={{ background: `${story?.bgcolor}` }}
      >
        {screenSize?.dynamicWidth > ipadMaxWidth && (
          <div>{animationImage(story)}</div>
        )}
        <h3
          style={{ color: story?.textcolor }}
          className={`${
            showDefaultContent ? "default-story-title" : "story-title"
          }`}
        >
          {story?.title}
        </h3>
        <p
          className={`${
            showDefaultContent ? "default-story-paragraph" : "story-paragraph"
          }`}
        >
          {story?.paragraph1}
        </p>
  ...
  return (
    <div className="slide-item-detail-page">
      {showDefaultContent ? (
        <div>
          {showFullViewImage ? (
            <div className="story-detail-info">{showContent(story)}</div>
          ) : (
            <div>{animationImage(story)}</div>
          )}
        </div>
