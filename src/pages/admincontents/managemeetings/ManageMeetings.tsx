...
import {
  convertoDate,
  isAllTimeSlotDeclined,
  isAnyOneTimeSlotInvited,
  showToast,
} from "../../../utils/utils";
import api from "../../../api";
...
export default function ManageMeetings() {
  ...
  const updateMeetingStatus = async (
    ...
    try {
      const result = await api.put(
        `/meeting-time-slot/${_timeslot?.id}`,
        payload
      );
      if (result.data.status === "success") {
        ...
        if (completeMeeting) {
          const result = await api.put(
            `/meeting-requested-user/${meeting?.id}`,
            payload
          );
          if (result.data.status === "success") {
            await fetchAllMeeting();
        ...
        if (status === "Declined") {
          const allTimeSlatsDecliened = result?.data?.payload.every(
            (obj: any) => obj.status === "Declined"
          );
  ...
  const handleInviteOrDecline = async (
    status: string,
    meeting: any,
    _timeslot: any
  ) => {
    if (_timeslot?.status !== "ACTIVE") {
      ...
      status === "Invited" &&
      meeting?.preferedDateAndTimeslots.some(
      ...
      if (
        status === "Declined" &&
        isAllTimeSlotDeclined(meeting?.preferedDateAndTimeslots)
      ) {
  ...
  const completeMeetingStatus = async (status: string, meeting: any) => {
    ...
    if (meeting?.status === "Completed") {
      ...
      if (isAnyOneTimeSlotInvited(meeting?.preferedDateAndTimeslots)) {
        ...
        showToast(
          "You can not complete the meeting without invite or decline the time slot.",
          false
        );
      }
    }
  ...
  const filteredMeetingsData = () => {
    return meetings.filter(
      (meeting: any) =>
        ...
        meeting.preferedDateAndTimeslots.some(
          (timeslot: any) => timeslot.status !== "Declined"
        )
    );
  };
      ...
      {filteredMeetingsData() && filteredMeetingsData()?.length > 0 ? (
        <div className="meetings-table">
          ...
          {filteredMeetingsData() &&
            ...
            filteredMeetingsData().map((meeting: any, index: any) => (
              <div key={meeting?.id || index}>
                <div className="row">
                  <div className="col-2 meetings-content">
                    {meeting?.name} {index + 1}
                  </div>
                  <div className="col-2 meetings-content">{meeting?.email}</div>
                  <div className="col-1 meetings-content">
                    {meeting?.type == "googlemeet" && <div>Google Meet</div>}
                    {meeting?.type == "zoom" && <div>Zoom</div>}
                    {meeting?.type == "msteams" && <div>MS Teams</div>}
                  </div>
                  <div className="col-2 meetings-content">
                    {meeting.preferedDateAndTimeslots.map(
                      (item: any, index: any) => (
                        <div key={index}>
                    ...
                    {meeting.preferedDateAndTimeslots.map((item: any) => (
                      <div key={item.id}>
                        <button
                          ...
                          onClick={() =>
                            handleInviteOrDecline("Invited", meeting, item)
                          }
                        >
                        ...
                        <button
                          ...
                          onClick={() =>
                            handleInviteOrDecline("Declined", meeting, item)
                          }
                        >
                  ...
                  <div className="col-2 meetings-content">
                    {meeting?.iscompleting ? (
                      ...
                      <button
                        ...
                        onClick={() =>
                          completeMeetingStatus("Completed", meeting)
                        }
                      >
                        {meeting?.status === "Completed"
                          ? "Completed"
